...
jest.mock('../../../src/models/tutorAction', () => {
  ...
  class FakeTutorList extends actual.ListarTarefasTutorAction {
    constructor(user: UserModel, idGato: string) { super(user, idGato); ctorMocks.tutorList(user, idGato); }
    run() { return runMocks.tutorList(); }
  }
  class FakeCreate extends actual.CriarTarefaTutorAction {
    constructor(user: UserModel, idGato: string, data: CreateTarefaInputDTO) { super(user, idGato, data); ctorMocks.create(user, idGato, data); }
    run() { return runMocks.create(); }
  }
  class FakeUpdate extends actual.AtualizarTarefaTutorAction {
    constructor(user: UserModel, idGato: string, idTarefa: string, data: UpdateTarefaInputDTO) { super(user, idGato, idTarefa, data); ctorMocks.update(user, idGato, idTarefa, data); }
    run() { return runMocks.update(); }
  }
  class FakeRemove extends actual.DeletarTarefaTutorAction {
    constructor(user: UserModel, idGato: string, idTarefa: string) { super(user, idGato, idTarefa); ctorMocks.remove(user, idGato, idTarefa); }
...
jest.mock('../../../src/models/catSitterAction', () => {
  ...
  class FakeCatSitterList extends actual.ListarTarefasCatSitterAction {
    constructor(user: UserModel, idGato: string) { super(user, idGato); ctorMocks.catSitterList(user, idGato); }
    run() { return runMocks.catSitterList(); }
  }
  class FakeConclude extends actual.ConcluirTarefa {
    constructor(user: UserModel, idTarefa: string) { super(user, idTarefa); ctorMocks.conclude(user, idTarefa); }
